...
const createSession = (id: string, startedAt: string, provider?: string): Session => {
...
describe('@llmscope/storage-memory', () => {
  it('stores, updates, filters, and clears sessions', async () => {
    ...
    const openai = await store.listSessions({ provider: 'openai', search: '/alpha' });
  ...
  it('evicts the least recently used session when over capacity', async () => {
    ...
    await store.saveSession(createSession('first', '2026-03-08T10:00:00.000Z', 'openai'));
    await store.saveSession(createSession('second', '2026-03-08T10:00:01.000Z', 'openai'));

    await store.getSession('first');

    await store.saveSession(createSession('third', '2026-03-08T10:00:02.000Z', 'openai'));
